...
<span style="color:red">TODO: THE SCREENSHOTS ARE NOT YET UP TO DATE AND THEIR LAYOUT IS NOT YET FINALIZED.</span>
...
The main `Puma` component is generating models, materials and reports out of a CityEngine Rule Package (RPK) and input shapes (polygons, surfaces, meshes).  The two helper components `Puma Reports Unpacker` and `Puma Reports Display` assist with inspecting and displaying report values generated by the CGA rules.

<img src="doc/img/man_gh_main_comp.jpg" height=200>
<img src="doc/img/man_gh_unpack_comp.jpg" height=200>
<img src="doc/img/man_gh_filter_comp.jpg" height=200>

#### Connecting the main Puma inputs

The main `Puma` component has two fixed inputs. The `RPK` input takes a file path to a CityEngine Rule Package file (.rpk). The second input `Shape(s)` takes one or more input geometries (Mesh, Rectangle, Brep, Surface).

<img src="doc/img/man_gh_init_shape.jpg" height=200>
<img src="doc/img/man_gh_select_rectangle.jpg" height=200>
<img src="doc/img/man_gh_select_rectangle_2.jpg" height=200>
...
Note that there are slight differences in these steps based on the geometry type. The `Surface` component, for example, does not allow to draw a shape, but only to select a previously existing one. It is needed to draw it first using Rhino tools.

#### Working with rule attribute inputs

<img src="doc/img/man_gh_rule_attributes.jpg" height=200>

When both main inputs are connected, the component is updated with additional inputs for the CGA rule attributes. These parameters can then be connected to other components. The context menu also provides an easy way to directly assign a value. Puma will use default values for unconnected inputs which are defined in the rules and in general also depend on the input shapes.
...
<img src="doc/img/man_gh_input_tooltips.jpg" height=200>
<img src="doc/img/man_gh_color_input.jpg" height=200>
...
<img src="doc/img/man_gh_enum_tooltip.jpg" height=200>
...
The `Puma` component has three outputs:

1. Models:  The generated meshes.
1. M: The generated materials
1. Reports:  The generated cga reports.

<img src="doc/img/man_gh_candler_result.jpg" height=200>

The `Models` output will contain for each input shape a list of meshes (one mesh per material).

<img src="doc/img/man_gh_apply_material.jpg" height=200>

The generated materials can be applied to the meshes by connecting the built-in `Custom Preview` component. The reports are output as a custom data type which can be displayed and converted with the Puma reports helper components.

#### Using the Puma reports helper components

<img src="doc/img/man_gh_filter_comp_connections.jpg" height=200>

<span style="color:red">TODO: add screenshot of rhino viewport with report values displayed.</span>

The `Puma Reports Display` is designed to help displaying CGA report values in the Rhino viewport and takes 5 inputs. The first two are typically connected to the respective outputs of the main `Puma` component. The next three inputs are optional filters:
...
<img src="doc/img/man_gh_filter_example.jpg" height=200>

This component has three outputs, the first two can be connected to a `3D Text Tag` component to display the selected reports in the Rhino viewport.  The `Reports Location` output provides the location to position and align the reports text above each generated model. The `Reports Display` output provides the formatted reports text. The `Reports` passes through the selected reports for further processing. For example, to unpack them with the `Puma Reports Unpacker`.

<img src="doc/img/man_gh_report_unpack_connected.jpg" height=200>
...
Please note, this command is only meant to provde a quick way for testing a RPK on a shape and it is currently not possible to change the rule attributes.
