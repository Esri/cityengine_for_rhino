...
const ON_Mesh GeneratedModel::toON_Mesh(const ModelPart& modelPart) const 
{
	...
	for (size_t v_id = 0; v_id < modelPart.mIndices.size(); ++v_id) {
		...
		mesh.SetVertexNormal(static_cast<int>(v_id), ON_3dVector(modelPart.mNormals[index * 3], modelPart.mNormals[index * 3 + 1], modelPart.mNormals[index * 3 + 2]));
	}
	...
	for (size_t i = 0; i < modelPart.mUVs.Count(); ++i) {
		mesh.SetTextureCoord(static_cast<int>(i), modelPart.mUVs[i].x, modelPart.mUVs[i].y);
	}
